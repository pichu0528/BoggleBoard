...
class BogglePlayer : public BaseBogglePlayer {
  public:
     ...
     * words).
     */
    void getWords(int row, int col, unsigned int minimum_word_length,
  ...
  private:
    ...
    /* resets visited member of each BoardPos to false*/
    void reset();

    /**
     * Helper function to find path for word for isOnBoard()
     */ 
    bool findWord(unsigned int row, unsigned int col, 
                  const std::string &word_to_check, unsigned int word_pos,
                  vector<int> *position);

    /**
     * Helper function to find the starting tile for isOnBoard()
     */ 
    bool findStart(unsigned int row, unsigned int col, 
                  const std::string &word_to_check, unsigned int word_pos,
                  vector<int> *position);    
